...
?>
    ...
    function view_tableLog($DTBL, $tblname, $primarykey = "", $condition = "", $order = "", $firstRow = 0, $totalRow = 100, $index = "")
    {
        $datatable = $DTBL->get_data($tblname, $condition, $order, $firstRow, $totalRow);
        $ddatatable = $datatable[0];
        $qdatatable = $datatable[1];
                        ...
                        if ($tdatatable > 0) {
                            do { ?>
                                <tr>
                                    ...
                                    ?>
                                    <?php do { ?>
                                        <?php if ($ddatatable_header2['Field'] === 'log_file') {
                                            if ($ddatatable[$ddatatable_header2['Field']] != '') {
                                                $link = $ddatatable[$ddatatable_header2['Field']];
                                                echo '<td class="text-center"><a class="btn btn-success" href="' . $link . '" target="_blank">view</a></td>';
                                            } else {
                                                echo '<td></td>';
                                            }
                                        } else {
        ...
        ?>
            <script>
                $(document).ready(function() {
                    <?php if (isset($params['dom0'])) {
                        echo $params['dom0'];
                    } ?>
                    var table = $('#<?php echo $params['nameView']; ?>').DataTable({
                        <?php echo $params['dom']; ?>;
                    });
                    <?php if (isset($params['dom2'])) {
                        echo $params['dom2'];
                    } ?>
                });
            </script>
