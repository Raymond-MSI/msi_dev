...
$mdlname = "new_request";
$DBTRXAPP = get_conn($mdlname);
// var_dump($DBTRXAPP);

$query = $DBTRXAPP->get_sqlV2("SELECT * from sa_trx_approval WHERE status_approval = 'Approved'");
while ($ambildata = mysqli_fetch_array($query[1])) {
    ...
    $insert = sprintf(
        "(`request`, `divisi`, `posisi`,`kode_project`) 
    VALUES ( %s, %s, %s, %s)",
    ...
    $res = $DBTRXAPP->insert_data('trx_share', $insert);
    // var_dump($res);
