<?php
// IMAP server details
// include_once 'C:\xampp\htdocs\microservices\imap-coba.php';
// include '../../../imap-coba.php';
            ...
            while ($view = mysqli_fetch_array($cekgm[1])) {
                ...
                if ($view['status_rekrutmen'] == 'Penambahan') {
                    if ($view['id_fpkb'] == $extractedPart && $view['status_gm'] == 'Pending') {
                        if ($gm == $from) {
                            if ($status_gm == "Disapprove" || $status_hcm == "Disapprove") {
                    ...
                    if ($view['id_fpkb'] == $extractedPart && $view['status_gm'] == 'Approve' && $view['status_gm_hcm'] == 'Pending') {
                        if ($gm_hcm == $from) {
                            ...
                            $updateResult = $DBHCM->get_res("UPDATE sa_hcm_requirement SET status_gm_hcm = '$status_hcm', catatan_gm_hcm = '$catatan_hcm' WHERE id_fpkb = '" . $extractedPart . "'");
                            if ($updateResult) {
                                $to = $view['gm_bod'];
                                $from = "MSIZone<msizone@mastersystem.co.id>";
                                $cc = "recruitment.team@mastersystem.co.id";
                    ...
                    if ($view['id_fpkb'] == $extractedPart && $view['status_gm'] == 'Approve' && $view['status_gm_hcm'] == 'Approve' && $view['status_gm_bod'] == 'Pending') {
                        if ($gm_hcm == $from) {
                            ...
                            $updateResult = $DBHCM->get_res("UPDATE sa_hcm_requirement SET status_gm_bod = '$status_bod', catatan_gm_bod = '$catatan_bod' WHERE id_fpkb = '" . $extractedPart . "'");
                            if ($updateResult) {
