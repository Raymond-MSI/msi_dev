...
if (isset($_POST['add'])) {
    $insert = sprintf(
        "(`id`,`divisi`,`posisi`,`jumlah_dibutuhkan`,`tanggal_dibutuhkan`,`deskripsi`,`nama_project`,`nama_customer`,`kode_project`,`status_rekrutmen`,`status_karyawan`,`mpp`,
    ...
    $data = $DBREC->get_sql("SELECT * FROM sa_trx_recruitement WHERE id=" . $_POST['id'] . "");
    $to = $data[0]['gm_hcm'];
    $from = "MSIZone<msizone@mastersystem.co.id>";
    $cc = $from;
    $bcc = "";
    ...
    if ($data[0]['gm'] == $_SESSION['Microservices_UserEmail']) {
        ...
        , `catatan`=%s, `diisi_oleh_hcm`=%s, `range_salary`=%s, `gm`=%s, `status_gm`=%s,`gm_hcm`=%s,`status_request`=%s",
            GetSQLValueString($_POST['id'], "int"),
            GetSQLValueString($_POST['divisi'], "text"),
            GetSQLValueString($_POST['posisi'], "text"),
            GetSQLValueString($_POST['jumlah_dibutuhkan'], "text"),
        ...
        $condition = "id=" . $_POST['id'];
        $update = sprintf(
            "`gm`=%s,`gm_hcm`=%s, `status_gm_hcm`=%s, `status_request`=%s",
            // GetSQLValueString($_POST['id'], "int"),
            // GetSQLValueString($_POST['divisi'], "text"),
            // GetSQLValueString($_POST['posisi'], "text"),
            // GetSQLValueString($_POST['jumlah_dibutuhkan'], "text"),
