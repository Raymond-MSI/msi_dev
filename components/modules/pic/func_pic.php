...
if (isset($_POST['add'])) {
    ...
    // exit();
    $insert = sprintf(
        "(`pic_name`,`pic_email`,`pic_address`,`pic_city`,`pic_phone`, `title`,`customer_code`,`survey_count`,`created_by`) VALUES (%s,%s,%s,%s,%s,%s,%s,0,%s)",
    ...
    $insertlog = sprintf(
        "(`pic_id`,`description`,`entry_by`) VALUES (%s,%s,%s)",
    ...
    if ($checkcust3 <= 0) {
        $insert2 = sprintf(
            "(`customer_code`,`customer_company_name`,`category`, `created_by`) VALUES (%s,%s,'Customer',%s)",
            GetSQLValueString($_POST['customer_code'], "text"),
    ...
    $condition = "pic_id=" . $_POST['pic_id'];
    $update = sprintf(
        "`pic_name`=%s,`pic_email`=%s,`pic_address`=%s,`pic_city`=%s,`pic_phone`=%s,`title`= %s, `customer_code` = %s",
        GetSQLValueString($_POST['pic_name'], "text"),
        GetSQLValueString($_POST['pic_email'], "text"),
        GetSQLValueString($_POST['pic_address'], "text"),
        GetSQLValueString($_POST['pic_city'], "text"),
