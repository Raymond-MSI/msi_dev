...
if (isset($_POST['save'])) {
    ...
    $insert = sprintf(
        "(`so_number`,`pic_approval`,`project_code`,`project_name`,`customer_name`,`no_bast`,`email_requester`,`error_list`,`error_list2`,`error_list3`,`error_list4`,`error_list5`,`error_list6`,`error_list7`,`error_list8`,`error_category`,`error_category2`,`error_category3`,`error_category4`,`error_category5`,`error_category6`,`error_category7`,`error_category8`,`notes`,`notes2`,`notes3`,`notes4`,`notes5`,`notes6`,`notes7`,`notes8`,`nilai_error`,`pic_email`,`status_approval`,`status_error`,`status`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE `pic_approval`=%s,`no_bast`=%s,`error_list`=%s,`error_list2`=%s,`error_list3`=%s,`error_list4`=%s,`error_list5`=%s,`error_list6`=%s,`error_list7`=%s,`error_list8`=%s,`error_category`=%s,`error_category2`=%s,`error_category3`=%s,`error_category4`=%s,`error_category5`=%s,`error_category6`=%s,`error_category7`=%s,`error_category8`=%s,`notes`=%s,`notes2`=%s,`notes3`=%s,`notes4`=%s,`notes5`=%s,`notes6`=%s,`notes7`=%s,`notes8`=%s,`nilai_error`=%s,`pic_email`=%s,`status_error`=%s,`status`=%s",
        GetSQLValueString($so_number, "text"),
    ...
    $insert = sprintf(
        "(`project_code`,`so_number`,`project_name`,`customer_name`,`no_bast`,`verif_status`,`pic_email`,`notes`,`status_approval`,`status`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE `no_bast`= %s,`verif_status`= %s,`pic_email`=%s,`notes`=%s,`status`=%s",
        GetSQLValueString($project_code, "text"),
        GetSQLValueString($so_number, "text"),
        GetSQLValueString($_POST['project_name'], "text"),
        GetSQLValueString($_POST['customer_name'], "text"),
    ...
    $insert = sprintf(
        "(`email_requester`,`pic_approval`,`so_number`,`project_code`,`project_name`,`customer_name`,`no_bast`,`verif_status`,`resources`,`pic_email`,`note`,`status_approval`,`status`,`tanggal_ic`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE `tanggal_ic`=%s,`no_bast`= %s,`verif_status`= %s,`resources`=%s,`pic_email`=%s,`pic_approval`=%s,`notes`=%s,`status`=%s",
        GetSQLValueString($_SESSION['Microservices_UserEmail'], "text"),
        GetSQLValueString($pic_name, "text"),
        GetSQLValueString($so_number, "text"),
        GetSQLValueString($project_code, "text"),
    ...
    if (isset($check_data[0]['pic_email'])) {
        ...
        if (isset($angka)) {
            ...
            if ($stts_pic[$nomor] == 'Pending') {
                ...
                $check_approval = $DBKPISO->get_sql("SELECT * FROM sa_kpi_board WHERE so_number='$so_number'");
                ...
                if (strpos($status_pic, "Pending") !== false) {
                    ...
                    $get_engineer3 = $DBWR->get_sql("SELECT DISTINCT a.resource_email from sa_view_dashboard_wrike_resource a left join sa_wrike_project_list b ON a.project_id=b.id where b.no_so='$so_number2'");
                    ...
                    if (isset($get_engineer3[0]['resource_email'])) {
                        ...
                        $msg .= "<tbody>";
                        do {
                            $id_project2 = $DBWR->get_sql("SELECT id from sa_wrike_project_list WHERE no_so='$so_number2' AND project_type='MSI Project Implementation'");
                            $idproject2 = isset($id_project2[0]['id']);
                            if (empty($idproject2)) {
                                $id2 = null;
                            } else {
                            ...
                            $get_roles = $DBWR->get_sql("SELECT roles FROM sa_resource_assignment WHERE resource_email LIKE '%" . $get_engineer3[0]['resource_email'] . "%' AND no_so='$so_number2'");
