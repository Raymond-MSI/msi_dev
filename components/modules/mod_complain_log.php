...
?>
        <script>
            $(document).ready(function() {
                var table = $('#log_complain').DataTable({
                    dom: 'Blfrtip',
                    select: {
                        style: 'single'
                    },
                    buttons: [{
                        ...
                        },
                        {
                            ...
                            action: function() {
                                ...
                                window.location.href = "index.php?mod=complain_log&act=view&complain_id=" + complain_id + "&submit=Submit";
                            },
                            enabled: false
                        },
                        {
                            ...
                            action: function() {
                                ...
                                var complain_id = table.cell(rownumber, 0).data();
                                window.location.href = "index.php?mod=complain_log&act=edit&complain_id=" + complain_id + "&submit=Submit";
                            }
                        },
                        {
                            text: "<i class='fa fa-plus'></i>",
                            action: function() {
                                window.location.href = "index.php?mod=complain_log&act=add";
                            },
                            // enabled: false
                        }
                    ],
        ...
        if ($_SESSION['Microservices_UserLevel'] == "Super Admin") {
            function view_data($tblname)
            {
                ...
                view_table($cobadb, $tblname, $primarykey, $condition, $order);
            }
            function form_data($tblname)
            {
                include("components/modules/complain_log/form_complain_log.php");
            }

            // End Function

            $database = 'sa_dashboard_kpi';
            include("components/modules/complain_log/connection.php");
            $DB = new Databases($hostname, $username, $userpassword, $database);
            $tblname = 'log_complain';

            include("components/modules/complain_log/func_complain_log.php");
