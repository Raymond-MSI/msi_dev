...
$mdlname = "new_request";
$DBREQ = get_conn($mdlname);


$query = $DBREQ->get_sqlV2("SELECT * from sa_trx_approval WHERE status_approval = 'Approved' AND assign is not null AND status = 'Active'");
while ($ambildata = mysqli_fetch_array($query[1])) {
    ...
    $update_request = sprintf("status = 'Send Email' ");
    $condition_request = "id =" .  $id_request;

    $insert = sprintf(
        "(`divisi`, `posisi`,`project_code`,`jumlah_dibutuhkan`) 
    VALUES (%s, %s, %s, %s)",
    ...
    $res = $DBREQ->insert_data('trx_share', $insert);

    $uprequest = $DBREQ->update_data("trx_approval", $update_request, $condition_request);
}
