...
$mdlname = "HCM";
$DBNR = get_conn($mdlname);
$query = $DBNR->get_sqlV2("SELECT * FROM sa_trx_request_requirement WHERE status_request = 'Submitted'");
while ($ambildata = mysqli_fetch_array($query[1])) {
    ...
    $res = $DBNR->insert_data('trx_request_approval', $insert);

    $upsurvey = $DBNR->update_data("trx_request_requirement", $update_request, $condition_request);
}
