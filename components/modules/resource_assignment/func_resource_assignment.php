...
if (isset($_POST['submitResourceApproval'])) {
    ...
    //if ($finalLeaderNameraw == 'Moch. Lintar Wahyu Wardana<lintar@mastersystem.co.id>') {
    //$finalLeaderName = 'Andino Holi Fajra<andino.hf@mastersystem.co.id>';
    //} elseif ($finalLeaderNameraw == 'lintar@mastersystem.co.id') {
    //$finalLeaderName = 'Andino Holi Fajra<andino.hf@mastersystem.co.id>';
    //} else {
    //  $finalLeaderName = $DBHCM->get_general_manager($finalResourceEmail);
    //}
    ...
    if ($_POST['status'] == "Penuh" && $_POST['startProgress'] != 0) {
        ...
        if (USERPERMISSION == "dbf36ff3e3827639223983ee8ac47b42" || USERPERMISSION == "7b7bc2512ee1fedcd76bdc68926d4f7b" || USERPERMISSION == "0162bce636a63c3ae499224203e06ed0") {
            ...
            $updateData = sprintf(
                "`status`= %s, `start_progress`= %s, `end_progress`= %s, `description` = %s, `modified_by` = %s, `modified_in_msizone`= %s",
        ...
        $conditionUpdate = "id=$idRow";
        $updateData = sprintf(
            "`status`= %s, `start_progress`= %s, `end_progress`= %s, `approval_status`= %s, `description` = %s, `modified_by` = %s, `modified_in_msizone`= %s",
            GetSQLValueString($_POST['status'], "text"),
            GetSQLValueString($_POST['startProgress'], "text"),
            GetSQLValueString($_POST['endProgress'], "text"),
    ...
    if ($totalRowTemporary > 0) {
        ...
        do {
            ...
            if ($finalLeaderNameraw == 'Moch. Lintar Wahyu Wardana<lintar@mastersystem.co.id>') {
                ...
                $finalLeaderName = 'Andino Holi Fajra<andino.hf@mastersystem.co.id>';
            } elseif ($finalLeaderNameraw == 'Johnny Widjaja<Johnny@mastersystem.co.id>') {
	    	$finalLeaderName = 'Iwan Rusmin<iwan@mastersystem.co.id>';
	    } elseif ($finalLeaderNameraw == 'Johnny@mastersystem.co.id') {
	    	$finalLeaderName = 'Iwan Rusmin<iwan@mastersystem.co.id>';
	    } else {
            ...
            if ($totalRowResource > 0) {
                ...
                $insertData = sprintf(
                    "(`project_code`, `project_id`, `order_number`, `no_so`, `project_type`, `customer_name`, `project_name`, `resource_email`, `roles`, `project_roles`, `start_progress`, `end_progress`, `status`, `description`, `created_by`, `modified_by`, `created_in_msizone`, `modified_in_msizone`, `approval_to`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
