...
function get_contact()
{
    ...
    curl_setopt($curl, CURLOPT_URL, 'https://www.wrike.com/api/v4/contacts?fields=["jobRoleId"]');
    ...
    for ($i = 0; $i < count($result1); $i++) {
        ...
        $jobRoles = $result1[$i]['jobRoleId'];

        $sqlLookupJobroles = "SELECT * FROM sa_wrike_config WHERE object_id = '$jobRoles'";
        $dataJobroles = $DBWR->get_sql($sqlLookupJobroles);
        $jobRolesName = $dataJobroles[0]['title'];
        ...
        if ($deleted == false) {
            if (strpos($type, 'Person') !== false) {
                ...
                if ($testdata[2] > 0) {
                    //Query Update
                    $update = sprintf(
                        "`name`='$first_name $last_name', `email`='$email', `role`='$role', `jobroles`='$jobRolesName'",
                    ...
                    //Query Insert
                    $insert = sprintf(
                        "(`id`,`name`,`email`, `role`, `jobroles`) VALUES ('$author_id', '$first_name $last_name', '$email', '$role', '$jobRolesName')",
...
function get_project()
{
    ...
    curl_setopt($curl, CURLOPT_URL, "https://www.wrike.com/api/v4/folders/IEAEOPF5I4U6PGE7/folders?project=true&fields=['customFields']&updatedDate={'start':'" . date('Y-m-d', strtotime('-1 days')) . "T00:00:00Z','end':'" . date('Y-m-d', strtotime('-1 days')) . "T23:59:59Z'}");
    //
    curl_setopt($curl, CURLOPT_HTTPHEADER, array('Content-Type: application/json', $authorization));
    curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
    $result = curl_exec($curl);
    ...
    for ($i = 0; $i < count($result1); $i++) {
        ...
        // $rowFlagDataJobroles = $flagdataJobroles[0];
        // $rFlagDataJobroles = $flagdataJobroles[0];
...
function getAssignment()
{
    ...
    if($totalRowAssignment > 0){
        do {
            ...
            if (is_countable($customFields) && count($customFields) > 0){
            ...
            //GET 5 Row BAST
            if (is_countable($resultPProject) && count($resultPProject) > 0){
                for($j = 0; $j < count($resultPProject); $j++){
                    ...
                    for($k = 0 ; $k < count($childIds); $k++){
                        ...
                        for($l = 0 ; $l < count($resultBastDate); $l++){
                            ...
                            if ($titleBast == '07. Bast document Date (actual)') {
                                $titleDocument = $resultBastDate[$l]['title'];
                                $documentBastDate = $resultBastDate[$l]['customFields'][0]['value'];
                            }
            ...
            echo "$title - $titleDocument - $documentBastDate <br/>";
            ...
            if ($projectType == 'MSI Project Implementation') {
                ...
                if ($dataProjectDetailImplementation[2] > 0) {
                    ...
                    $updateProjectDetailImplementation = sprintf(
                        ...
                        `kom_bast_date` = '$komBastDate', `po_bast_date` = '$poBastDate', `contract_bast_date` = '$contractBastDate', `cr_bast_date` = '$crBastDate', `addendum_bast_date` = '$addendumBastDate', `document_bast_date` = '$documentBastDate', `project_type`='$projectType', `project_category`='$projectCategory', `product`='$finalProduct'",
                        GetSQLValueString($projectCode, "text"),
                        GetSQLValueString($taskId, "text"),
                        GetSQLValueString($taskName, "text"),
                        GetSQLValueString($responsibleId, "text"),
                    ...
                    $insertProjectDetailImplementation = sprintf(
                        "(`project_id`, `project_code`, `customer_name`, `start_date_project`, `finish_date_project`, `sbf_bast_date`, `kom_bast_date`, `po_bast_date`, `contract_bast_date`, `cr_bast_date`, `addendum_bast_date`, `document_bast_date`, `project_type`, `project_category`, `product`) 
                        VALUES ('$projectId', '$projectCode', '$customerName', '$startDate', '$endDate', '$sbfBastDate', '$komBastDate', '$poBastDate', '$contractBastDate', '$crBastDate', '$addendumBastDate', '$documentBastDate', '$projectType', '$projectCategory', '$finalProduct')",
                        GetSQLValueString($projectId, "text"),
                        GetSQLValueString($projectCode, "text"),
                ...
                if ($dataProjectDetailMaintenance[2] > 0) {
                    ...
                    $updateProjectDetailMaintenance = sprintf(
                        "`project_code`='$projectCode', `customer_name`='$customerName', `start_date_project`='$startDate', `finish_date_project`='$endDate', `sbf_bast_date` = '$sbfBastDate', 
                        `kom_bast_date` = '$komBastDate', `po_bast_date` = '$poBastDate', `contract_bast_date` = '$contractBastDate', `cr_bast_date` = '$crBastDate', `addendum_bast_date` = '$addendumBastDate', `document_bast_date` = '$documentBastDate', `project_type`='$projectType', `project_category`='$projectMaintenanceCategory', `product`='$finalProduct'",
                    ...
                    $insertProjectDetailMaintenance = sprintf(
                        "(`project_id`, `project_code`, `customer_name`, `start_date_project`, `finish_date_project`, `sbf_bast_date`, `kom_bast_date`, `po_bast_date`, `contract_bast_date`, `cr_bast_date`, `addendum_bast_date`, `document_bast_date`, `project_type`, `project_category`, `product`) 
                        VALUES ('$projectId', '$projectCode', '$customerName', '$startDate', '$endDate', '$sbfBastDate', '$komBastDate', '$poBastDate', '$contractBastDate', '$crBastDate', '$addendumBastDate', '$documentBastDate', '$projectType', '$projectMaintenanceCategory', '$finalProduct')",
                        GetSQLValueString($projectId, "text"),
                        GetSQLValueString($projectCode, "text"),
                        GetSQLValueString($customerName, "text"),
                        GetSQLValueString($startDate, "text"),
