...
include($rootfolder . "applications/connections/connections.php");
include($rootfolder . "components/classes/func_databases_v3.php");
include($rootfolder . "components/classes/func_cfg_web.php");
include($rootfolder . "components/classes/func_component.php");
include($rootfolder . "components/classes/func_modules.php");
...
if (isset($_POST['act']) && $_POST['act'] == 'getProjectName') {
    ...
    if ($_POST['source'] == 'kpi') {
        if ($_POST['type'] == 'MSI Project Implementation') {
            $query = 'select project_name, SB_project_category from sa_data_so where order_number = "' . $_POST['order_number'] . '"';
            $data = $DBKPI->get_sql($query);
            $data1 = $data[0];
            $data2 = $data[1];
            $proj_name = $data1['project_name'];
        ...
        if ($_POST['type'] == 'MSI Project Maintenance') {
            $query = 'select project_name, tos_id from sa_trx_project_implementations imp join sa_trx_project_list trx on imp.project_id = trx.project_id where trx.order_number = "' . $_POST['order_number'] . '" AND service_type = 2';
            $data = $DBSB->get_sql($query);
            $data1 = $data[0];
            $data2 = $data[1];
            $proj_name = $data1['project_name'];
    ...
    if ($_POST['source'] == 'sbf') {
        if ($_POST['type'] == 'MSI Project Implementation') {
            $query = 'select project_name, tos_category_id from sa_trx_project_implementations imp join sa_trx_project_list trx on imp.project_id = trx.project_id where trx.order_number = "' . $_POST['order_number'] . '" AND service_type = 1';
            $data = $DBSB->get_sql($query);
            $data1 = $data[0];
            $data2 = $data[1];
            $proj_name = $data1['project_name'];
    ...
    $querycust = 'select customer_code, customer_name from sa_trx_project_list where order_number = "' . $_POST['order_number'] . '"';
    $cust = $DBSB->get_sql($querycust);
    $custcode = $cust[0]['customer_code'];
    $custname = $cust[0]['customer_name'];
    echo $custcode . '|' . $proj_name . '|' . $tos . '|' . $custname;
...
if (isset($_POST['act']) && $_POST['act'] == 'getProjectDetail') {
    ...
    $source = $_POST['source'];
    if ($source == 'kpi' && $order_number != '') {
        $mdlname = "KPI_BOARD";
        $DB1 = get_conn($mdlname);
        // $query = 'select if(so.SB_service_type_implementation != "0",so.SB_project_category, null) as implementation, so.SB_service_type_maintenance as maintenance from sa_kpi_board kpi join sa_data_so so on kpi.so_number = so.so_number_kpi where kpi.verif_status = "Completed;" AND kpi.so_number = "' . $so_number . '"';
        $query = 'select if(so.SB_service_type_implementation != "0",so.SB_project_category, null) as implementation, so.SB_service_type_maintenance as maintenance from sa_kpi_board kpi join sa_data_so so on kpi.order_number = so.order_number where kpi.verif_status = "Completed;" AND kpi.order_number = "' . $order_number . '"';
        $data = $DB1->get_sql($query);
        $data1 = $data[0];
        $data2 = $data[1];
        $sbtype = '';
    ...
    if ($source == 'sbf' && $order_number != '') {
        $mdlname = "SERVICE_BUDGET";
        $DB1 = get_conn($mdlname);
        // $query = 'select bundling from sa_trx_project_list where so_number = "' . $so_number . '" and status IN ("acknowledge", "approved")';
        $query = 'select bundling from sa_trx_project_list where order_number = "' . $order_number . '" and status IN ("acknowledge", "approved")';
        $data = $DB1->get_sql($query);
        $data1 = $data[0];
        $data2 = $data[1];
        $sbtype = '';
