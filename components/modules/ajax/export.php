...
$gettemplate = "SELECT * FROM sa_question_template where valid_year = " . $year;
$temp_list = $DB1->get_sql($gettemplate);
$template = $temp_list[0];
$nexttemp = $temp_list[1];
$sheet = 0;
print_r($template);
exit();
do {
...
$myWorkSheet = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, $template['template_name']);
...
$query = "SELECT surv.survey_id, so_number, pic_name, pic_email, pic_phone, customer_company_name, trx.project_code, trx.project_title,
        ...
        where YEAR(surv.created_datetime) = " . $year ." AND surv.main_template_id = '".$template['template_id']."'";
...
}while($template = $nexttemp->fetch_assoc());
ob_clean();
...
header("Content-Disposition: attachment; filename=List Hasil Survey Tahun $year.xlsx");
