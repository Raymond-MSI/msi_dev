...
if ($roles != '' && $roles != ' - ') {
    $sqlCheckRes = "SELECT * FROM sa_resource_assignment WHERE resource_email = '$email' AND project_id = '$projectId' AND roles = '$roles' AND status = '$status'";
    ...
    if ($rowCountRes > 0) {
        ...
        $sqlCheck = "SELECT * FROM sa_temporary_resource WHERE resource_email = '$email' AND project_id = '$projectId' AND roles = '$roles' AND status = '$status'";
    ...
    echo "<script>alert('Ada field yang kosong IMP ! " . $projectCode . "');</script>";
}
