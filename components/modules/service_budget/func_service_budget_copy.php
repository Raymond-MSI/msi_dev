...
if(isset($_POST['save_copy_service_budget'])) {
    for($i=0; $i<count($_POST['project_code']); $i++) {
        if(isset($_POST['check'][$i])) {
            ...
            if($orgData[2]) {
                do {
                    $mysql = sprintf("(`project_id`, `solution_name`, `product`, `services`) VALUES (%s,%s,%s,%s)",
            ...
            $mysql =sprintf("(`approve_by`, `approve_note`,`approve_status`, `project_id`) VALUES (%s,%s,%s,%s)",
                GetSQLValueString($_SESSION['Microservices_UserEmail'], "text"),
            ...
            $mysql = sprintf("(`project_id`, `description`, `entry_by`) VALUES (%s,%s,%s)",
