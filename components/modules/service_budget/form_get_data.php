sta<?php
    ...
    if ($_GET['act'] == 'edit') {
        ...
        //     $po_maintenance = $row['SB_maintenance_price'];
        //     $customer_name = $row['customer_name'];
        //     if ($po_maintenance == null) {
        //         $po_maintenance = 0;
        //     } else {
        //         $po_maintenance = number_format($row['SB_maintenance_price'], 0, ".", "");
        //     }
        //     $po_warranty = $row['SB_warranty_price'];
        //     if ($po_warranty == null) {
        //         $po_warranty = 0;
        //     } else {
        //         $po_warranty = $row['SB_warranty_price'];
        ...
        //     if ($implementation_price == null) {
        //         $implementation_price = 0;
        //     } else {
        //         $implementation_price = number_format($row['SB_amount_idr'], 0, ".", "");
        //     }
        //     $start_date_project = $row['WR_start_assignment_implementation'];
        //     $bast_date_project = $row['WR_bast_date_actual_project_implementation'];
        //     $bast_plan = $row['WR_bast_date_project_implementation'];
        ...
        //         $commercial_result = 0;
        //     }
        //     if ($mandays_actual_wr == null && $duration_actual_wr == null) {
        //         $commercial_category = 'Empty';
        //         $commercial_result = 0.0;
        //         $timeline_category = 'Empty';
        //         $timeline_result = 0.0;
        //     } else if ($mandays_actual_wr == null && $duration_actual_wr != null) {
        //         $commercial_category = 'Empty';
        //         $commercial_result1 = 0.0;
        //         $commercial_result = (float)number_format($commercial_result1, 2, ".", ",");
        //         var_dump($commercial_result);
        //     } else if ($duration_actual_wr == null && $mandays_actual_wr != null) {
        //         $timeline_category = 'Empty';
        //         $timeline_result = 0.0;
        //     }
        ...
        //     $status_wr = $row['WR_status_project'];
        //     $status_project = $row['status'];
        //     $sqll = $DBSB->get_sql("SELECT project_code,project_name_internal FROM sa_trx_project_list WHERE project_code='$project_code' GROUP BY project_code");
        //     $project_name = $sqll[0]['project_name_internal'];

        //     $mysql = sprintf(
        //         "(`project_code`, `customer_name`, `project_name`, `value`, `start_assignment`, `bast_plan`, `bast_actual`,`total`,`time_category`,`time_kpi`,`cte`,`total_cte`,`max_value`,`weighted_value`,`commercial_category`,`commercial_kpi`,`periode`,`status_wr`,`status_project`,`error_category`,`error_kpi`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE `value` = %s, start_assignment = %s, bast_plan= %s, bast_actual = %s, total = %s, time_category = %s, time_kpi = %s, cte = %s, total_cte = %s, max_value = %s, weighted_value = %s, commercial_category = %s, commercial_kpi = %s, periode = %s, status_wr = %s, status_project= %s, error_kpi=%s",
        ...
        //         GetSQLValueString(number_format($error_result, 2, ".", ""), "text")
        //     );
        //     $DBKPI->insert_data("kpi_project_wr", $mysql);
        //     // ON DUPLICATE KEY UPDATE `value` = $value, start_assignment = $start_date_project, bast_plan= $bast_plan, bast_actual = $bast_date_project, total = $estimation_wr, time_category = $timeline_category, time_kpi = $timeline_result, cte = $cte_project, total_cte = $cte, max_value = $max_value, weighted_value = $weighted_value, commercial_category = $commercial_category, commercial_kpi = $commercial_result, periode = $periode, status_wr = $status_wr, status_project= $status_project
        //     // $insert_data = $DBKPI->get_sql("INSERT IGNORE INTO sa_kpi_project_wr (project_code,customer_name,project_name,value,start_assignment,bast_plan,bast_actual,total,time_category,time_kpi,cte,total_cte,max_value,weighted_value,commercial_category,commercial_kpi,periode,status_wr,status_project,error_category,error_kpi) VALUES ('$project_code','$customer_name','$project_name','$value','$start_date_project','$bast_plan','$bast_date_project','$estimation_wr','$timeline_category','$timeline_result','$cte_project','$cte','$max_value','$weighted_value','$commercial_category','$commercial_result','$periode', '$status_wr', '$status_project','$error_category','$error_result') ON DUPLICATE KEY UPDATE value = '$value', start_assignment = '$start_date_project', bast_plan='$bast_plan', bast_actual = '$bast_date_project', total = '$duration_actual_wr', time_category = '$timeline_category', time_kpi = '$timeline_result', cte = '$cte_project', total_cte = '$cte', max_value = '$max_value', weighted_value = '$weighted_value', commercial_category = '$commercial_category', commercial_kpi = '$commercial_result', periode = '$periode', status_wr = '$status_wr', status_project= '$status_project'");
        ...
        do {
            ...
            //     $timeline_category = 'Critical';
            //     $timeline_result = $param1 * $param2;
            // } else if ($timeline >= 30 && $timeline < 90) {
            //     $get_param = $DBKPI->get_sql("SELECT value,value_type FROM sa_params WHERE category='Minor' AND type='Timeline'");
            //     $param1 = $get_param[0]['value'];
            //     $param2 = $get_param[0]['value_type'];
            //     $timeline_category = 'Minor';
        ...
        do {
            ...
            $get_ideal_project = $DBKPI->get_sql("SELECT so_number,max_value,total_cte,weighted_value,periode FROM sa_kpi_so_wr WHERE so_number='$no_so'");
            $max_value = $get_ideal_project[0]['max_value'];
            $total_cte = $get_ideal_project[0]['total_cte'];
            ...
            // $execute = $DBWR->get_sql($sql);
            // $sqlmode = $DBWR->get_sql("SET SQL_MODE='ALLOW_INVALID_DATES'");
            // $get_productivity_plan = $DBWR->get_sql("SELECT COUNT(resource_email) as total FROM sa_view_task_assignment WHERE resource_email LIKE '%$resource_email%'");
            // $get_productivity_actual = $DBWR->get_sql("SELECT COUNT(resource_email) as total_actual FROM sa_view_dashboard_wrike_resource WHERE resource_email LIKE '%$resource_email%'");
            // $task_plan = $get_productivity_plan['total'];
            // $task_actual = $get_productivity_actual['total_actual'];
            // $productivity = $task_plan / $task_actual;
            $nilai_akhir_ideal = $max_value * ($progress_final / 100);
            $nilai_akhir_aktual = $nilai_akhir_ideal * $total_cte;
            $nama_email = $nama[0] . " <" . $nama[1];
            ...
            $project_name = $get_status[0]['project_name'];
            $customer_name = $get_status[0]['customer_name'];
            $check_data = $DBKPI->get_sql("SELECT Nama,project_code,so_number FROM sa_user WHERE Nama LIKE '%$nama_email%' AND so_number = '$no_so'");
            if (empty($check_data[0]['Nama'])) {
                $insert_data_user_kpi = $DBKPI->get_sql("INSERT IGNORE INTO sa_user (Nama,project_code,so_number,project_name,customer_name,role,nilai_ideal,nilai_aktual,progress_start,progress_end,progress,project_support,cte,nilai_akhir_ideal,nilai_akhir_aktual,status_wr,periode,periode_so,status_project,kpi_status) VALUES ('$nama_email','$project_code_wr','$so_number','$project_name','$customer_name','$roles','$max_value','$weighted_value','$satu','$dua','$progress_final','$status','$total_cte','$nilai_akhir_ideal','$nilai_akhir_aktual','$status_wr','$periode','$periode_so','$status_project','$kpi_status')");
            } else {
                $nama = $check_data[0]['Nama'];
                $so = $check_data[0]['so_number'];
                $update_data_user_kpi = $DBKPI->get_sql("UPDATE sa_user SET role='$roles',progress_start='$satu',progress_end='$dua',progress='$progress_final',project_support='$status',nilai_akhir_ideal='$nilai_akhir_ideal',nilai_akhir_aktual='$nilai_akhir_aktual',status_wr='$status_wr',status_project='$status_project',kpi_status='$kpi_status' WHERE Nama LIKE '%$nama%' AND so_number='$so'");
            }
        ...
        do {
            ...
            // $project_id = $sql_wr_user[0]['project_id'];
            $project_type = $sql_wr_user[0]['project_type'];
            $so_number = $sql_wr_user[0]['no_so'];
            // $get_data_so = $DBWR->get_sql("SELECT a.project_id,b.no_so from sa_view_dashboard_wrike_resource a left join sa_wrike_project_list b ON b.id=a.project_id WHERE b.id='$project_id'");
            // $so_number = $get_data_so[0]['no_so'];
            ...
            $status_project = $get_status[0]['status'];
            $kpi_status = $get_status[0]['kpi_status'];
            $get_ideal_project = $DBKPI->get_sql("SELECT max_value,total_cte,weighted_value,periode,periode_so FROM sa_kpi_so_wr WHERE so_number LIKE '%$so_number%'");
            $max_value = $get_ideal_project[0]['max_value'];
            if (empty($max_value)) {
                $max_value = 0;
            } else {
            ...
            $check_data = $DBKPI->get_sql("SELECT Nama,project_code,so_number FROM sa_user WHERE Nama LIKE '%$nama_email%' AND so_number = '$so_number' AND role LIKE '%$resource_role%'");
            // echo $so_number;
            // echo $status_project . "<br>";
            // echo $status_wr . "<br>";
            if (empty($check_data[0]['Nama'])) {
                // echo $max_value . "<br>" . $total_cte . "<br>" . $weighted_value . "<br>";
                $insert_data_user_kpi = $DBKPI->get_sql("INSERT INTO sa_user (Nama,project_code,customer_name,so_number,project_type,project_name,role,nilai_ideal,nilai_aktual,start_assignment,end_assignment,duration,cte,status_wr,periode,periode_so,status_project,kpi_status) VALUES ('$nama_email','$project_code','$customer_name','$so_number','$project_type','$project_name','$resource_role','$max_value','$weighted_value','$start_assignment','$end_assignment','$hasil_durasi','$total_cte','$status_wr','$periode','$periode_so','$status_project','$kpi_status')");
            } else {
                $nama = $check_data[0]['Nama'];
                $so_number = $check_data[0]['so_number'];
                $update_sql = $DBKPI->get_sql("UPDATE sa_user SET kpi_status='$kpi_status', so_number='$so_number',nilai_ideal='$max_value',nilai_aktual='$weighted_value',start_assignment='$start_assignment',end_assignment='$end_assignment',duration='$hasil_durasi',cte='$total_cte',status_wr='$status_wr',periode='$periode',status_project='$status_project' WHERE Nama LIKE '%$nama%' AND so_number='$so_number'");
            }
            // $insert = $DBKPI->get_sql("INSERT INTO sa_user (Nama,project_code,so_number,role,nilai_ideal,nilai_aktual,start_assignment,end_assignment,duration,status_wr,status_project) VALUES ('$nama_email','$project_code','$so_number','$resource_role','$start_assignment','$end_assignment','$hasil_durasi','$status_wr','$status_project')");
        } while ($sql_wr_user[0] = $sql_wr_user[1]->fetch_assoc());

        // $sql_wr = "SELECT project_code,resource_email,roles,status,progress FROM `sa_view_resource_assignment`";
        // mysqli_select_db($conn_WR, 'sa_view_resource_assignment');
        // $retval_wr = mysqli_query($conn_WR, $sql_wr);
        // if (!$retval_wr) {
        //     die('Could not get data: ' . mysqli_error());
        // }
        // while ($row_wr = mysqli_fetch_assoc($retval_wr)) {
        //     $project_code_wr = $row_wr['project_code'];
        //     $resource_email = $row_wr['resource_email'];
        //     $roles = $row_wr['roles'];
        //     $status = $row_wr['status'];
        //     $progress = $row_wr['progress'];
        //     $get_ideal_project = $DBKPI->get_sql("SELECT max_value,total_cte,weighted_value,periode FROM sa_kpi_project_wr WHERE project_code='$project_code_wr'");
        //     $max_value = $get_ideal_project[0]['max_value'];
        //     $total_cte = $get_ideal_project[0]['total_cte'];
        //     $periode = $get_ideal_project[0]['periode'];
        //     $weighted_value = $get_ideal_project[0]['weighted_value'];
        //     $get_data_pic = "SELECT start_actual FROM `sa_view_dashboard_wrike_resource` WHERE resource_email='$resource_email' AND project_code='$project_code_wr' ORDER BY start_actual ASC";
        //     mysqli_select_db($conn_WR, 'sa_view_dashboard_wrike_resource');
        //     $retval_wr2 = mysqli_query($conn_WR, $get_data_pic);
        //     $row_wr2 = mysqli_fetch_assoc($retval_wr2);
        //     $start_actual = $row_wr2['start_actual'];
        //     $get_data_pic2 = "SELECT finish_actual FROM `sa_view_dashboard_wrike_resource` WHERE resource_email='$resource_email' AND project_code='$project_code_wr' ORDER BY finish_actual DESC";
        //     mysqli_select_db($conn_WR, 'sa_view_dashboard_wrike_resource');
        //     $retval_wr3 = mysqli_query($conn_WR, $get_data_pic2);
        //     $row_wr3 = mysqli_fetch_assoc($retval_wr3);
        //     $finish_actual = $row_wr3['finish_actual'];
        //     $get_data_so = "SELECT a.project_code,a.project_status,b.no_so from sa_wrike_project_detail a left join sa_wrike_project_list b on a.project_id = b.id where a.project_code LIKE '%$project_code_wr%' GROUP BY b.no_so";
        //     mysqli_select_db($conn_WR, 'sa_view_dashboard_wrike_resource');
        //     $retval_wr4 = mysqli_query($conn_WR, $get_data_so);
        //     $row_wr4 = mysqli_fetch_assoc($retval_wr4);
        //     $so_number = $row_wr4['no_so'];
        //     $result = preg_replace("/%/", "", $progress);
        //     $hobi = explode("-", $result);
        //     $satu = $hobi[0];
        //     $dua = $hobi[1];
        ...
        //     $duration = $difference->days;
        //     $hasil_durasi = $duration + 1;
        //     $nama_orang = "SELECT employee_name FROM sa_view_employees WHERE employee_email='$resource_email'";
        //     mysqli_select_db($conn_HCM, 'sa_view_employees');
        //     $retval_wr4 = mysqli_query($conn_HCM, $nama_orang);
        //     $row_wr4 = mysqli_fetch_assoc($retval_wr4);
        //     $nama = $row_wr4['employee_name'];
        //     $nama_email = $nama . " <" . $resource_email . ">";
        //     $get_status = $DBKPI->get_sql("SELECT WR_status_project,status FROM sa_data_so WHERE project_code_kpi LIKE '%$project_code_wr%'");
        //     $status_wr = $get_status[0]['WR_status_project'];
        //     $status_project = $get_status[0]['status'];
        //     $check_data = $DBKPI->get_sql("SELECT Nama,project_code FROM sa_user WHERE Nama LIKE '%$nama_email%' AND project_code = '$project_code_wr'");
        //     // echo $so_number;
        ...
        //     // echo $status_wr . "<br>";
        //     if (empty($check_data[0]['Nama'])) {
        //         $insert_data_user_kpi = $DBKPI->get_sql("INSERT INTO sa_user (Nama,project_code,so_number,role,nilai_ideal,nilai_aktual,start_assignment,end_assignment,duration,progress,project_support,cte,nilai_akhir_ideal,nilai_akhir_aktual,status_wr,periode,status_project) VALUES ('$nama_email','$project_code_wr','$so_number','$roles','$max_value','$weighted_value','$start_actual','$finish_actual','$hasil_durasi','$progress_final','$status','$total_cte','$nilai_akhir_ideal','$nilai_akhir_aktual','$status_wr','$periode','$status_project')");
        //     } else {
        //         echo 'Data Sudah Ada';
        //     }
        // }
        ...
        do {
            ...
            if (empty($check_data[0]['Nama'])) {
                $insert_data_user_kpi = $DBKPI->get_sql("INSERT IGNORE INTO sa_summary_user (Nama,project,personal_assignment,nilai_project,nilai_personal_assignment,total_nilai,produktifitas,status_wr,periode,periode_so,status_project,kpi_status) VALUES ('$nama','$project','$personal_assignment','$nilai_ideal_project','$nilai_ideal_personal_assignment','$total_nilai_ideal','$produktifitas','$status_wr', '$periode','$periode_so','$status_project','$kpi_status')");
            } else {
                echo "data sudah ada";
                //$update_data = $DBKPI->get_sql("UPDATE sa_summary_user SET project='$project', personal_assignment='$personal_assignment',nilai_project='$nilai_ideal_project',nilai_personal_assignment='$nilai_ideal_personal_assignment',total_nilai='$total_nilai_ideal',produktifitas='$produktifitas',status_wr='$status_wr',periode='$periode',status_project='$status_project' WHERE Nama LIKE '%$nama%' AND kpi_status='Not yet reviewed'");
            }
        ...
        do {
            ...
            $start_actual = $get_data_pic[0]['start_actual'];
            $update_data_user_kpi1 = $DBKPI->get_sql("UPDATE sa_user SET start_assignment='$start_actual' WHERE Nama LIKE '%$nama_resource%' AND so_number='$no_so'");
        ...
        do {
            ...
            $finish_actual = $get_data_pic2[0]['finish_actual'];
            $update_data_user_kpi = $DBKPI->get_sql("UPDATE sa_user SET end_assignment='$finish_actual' WHERE Nama LIKE '%$nama_resource%' AND so_number='$no_so'");
        ...
        do {
            ...
            if (is_numeric($tahun)) {
                $sql_update_periode = $DBKPI->get_sql("UPDATE sa_data_so SET periode_so=$tahun WHERE so_number_kpi='$so_number'");
                $sql_update_periode2 = $DBKPI->get_sql("UPDATE sa_kpi_so_wr SET periode_so=$tahun WHERE so_number='$so_number'");
                $sql_update_periode3 = $DBKPI->get_sql("UPDATE sa_user SET periode_so=$tahun WHERE so_number='$so_number'");                
            } else {
                $sql_update_periode = $DBKPI->get_sql("UPDATE sa_data_so SET periode_so=2022 WHERE so_number_kpi='$so_number'");
                $sql_update_periode2 = $DBKPI->get_sql("UPDATE sa_kpi_so_wr SET periode_so=2022 WHERE so_number='$so_number'");
                $sql_update_periode3 = $DBKPI->get_sql("UPDATE sa_user SET periode_so=2022 WHERE so_number='$so_number'");
            }
        ...
        do {
            ...
            if ($no_so == null) {
                ...
                $update_time = $DBKPI->get_sql("INSERT INTO sa_data_so (so_number_kpi,WR_created_at) VALUES ('$no_so','$created_at') WHERE so_number_kpi='$no_so' ON DUPLICATE KEY UPDATE WR_created_at='$created_at'");
            }
        } while ($get_time[0] = $get_time[1]->fetch_assoc());

        // $sql = $DBSB->get_sql("SELECT project_id,project_code,project_name_internal,customer_name,bundling, SUM(amount_idr) AS total_amount_idr, SUM(amount_usd) AS total_amount_usd FROM sa_trx_project_list WHERE status = 'acknowledge' AND bundling LIKE '%1%' OR status='acknowledge' AND bundling LIKE '%2%' OR status='acknowledge' AND bundling LIKE '%3%' GROUP BY project_code ORDER BY project_id");
        // $no = 1;
        // do {
        //     $project_id = $sql[0]['project_id'];
        //     $project_code_sb = $sql[0]['project_code'];
        //     $project_name_sb = $sql[0]['project_name_internal'];
        //     $customer_name_sb = $sql[0]['customer_name'];
        //     $bundling_sb = $sql[0]['bundling'];
        //     $amount_idr_sb = $sql[0]['total_amount_idr'];
        //     $amount_usd_sb = $sql[0]['total_amount_usd'];
        //     $status = 'Open';
        //     $kpi_status = 'Open';
        //     // $project_name_sb = preg_replace("[ ]", "", $project_name);
        //     // echo $project_name_sb . "<br>";
        //     $insert_data_pl = $DBKPI->get_sql("INSERT INTO sa_data_kp (project_id,project_code_kpi,project_name,customer_name,SB_bundling,SB_amount_idr,SB_amount_usd,WR_status_project,kpi_status) VALUES ('$project_id','$project_code_sb','$project_name_sb','$customer_name_sb','$bundling_sb','$amount_idr_sb','$amount_usd_sb','$status','$kpi_status') ON DUPLICATE KEY UPDATE SB_amount_idr='$amount_idr_sb', SB_amount_usd='$amount_usd_sb'");
        //     // $insert_data_pl = $DBKPI->get_sql("INSERT INTO sa_data_kp (project_id,project_code_kpi,customer_name,SB_bundling,SB_amount_idr,SB_amount_usd,WR_status_project) VALUES ('$project_id','$project_code_sb','$customer_name_sb','$bundling_sb','$amount_idr_sb','$amount_usd_sb','$status') ON DUPLICATE KEY UPDATE SB_amount_idr='$amount_idr_sb', SB_amount_usd='$amount_usd_sb'");
        // } while ($sql[0] = $sql[1]->fetch_assoc());
        ...
        do {
            ...
            $status = 'Open';
            $kpi_status = 'Open';
            $insert_data_pl = $DBKPI->get_sql("INSERT INTO sa_data_so (project_id,project_code_kpi,so_number_kpi,project_name,customer_name,SB_bundling,SB_amount_idr,SB_amount_usd,WR_status_project,kpi_status) VALUES ('$project_id','$project_code_sb','$so_number','$project_name_sb','$customer_name_sb','$bundling_sb','$amount_idr_sb','$amount_usd_sb','$status','$kpi_status') ON DUPLICATE KEY UPDATE SB_amount_idr='$amount_idr_sb', SB_amount_usd='$amount_usd_sb'");
            // $insert_data_pl = $DBKPI->get_sql("INSERT INTO sa_data_so (project_id,project_code_kpi,so_number_kpi,customer_name,SB_bundling,SB_amount_idr,SB_amount_usd,WR_status_project) VALUES ('$project_id','$project_code_sb','$so_number','$customer_name_sb','$bundling_sb','$amount_idr_sb','$amount_usd_sb','$status') ON DUPLICATE KEY UPDATE SB_amount_idr='$amount_idr_sb', SB_amount_usd='$amount_usd_sb'");
        ...
        do {
            ...
            $insert_data_pi_im = $DBKPI->get_sql("INSERT INTO sa_data_so (project_code_kpi,so_number_kpi,SB_service_type_implementation, SB_project_category, SB_project_estimation_implementation, SB_implementation_price, SB_bpd_price_implementation) VALUES ('$project_code_sb2','$so_number',1,'$tos_category_id_sb','$DAY',$total_implementation_price_sb,$bpd_price_sb) ON DUPLICATE KEY UPDATE SB_service_type_implementation=1, SB_project_category='$tos_category_id_sb', SB_project_estimation_implementation='$DAY', SB_implementation_price=$total_implementation_price_sb, SB_bpd_price_implementation=$bpd_price_sb");
        ...
        do {
            ...
            // echo "++" . $mandays_imp;
            $insert_data_man_imp = $DBKPI->get_sql("INSERT INTO sa_data_so (project_code_kpi,so_number_kpi,SB_mandays_implementation) VALUES ('$project_code_sb_mandays','$so_number','$mandays_imp') ON DUPLICATE KEY UPDATE SB_mandays_implementation='$mandays_imp'");
        ...
        do {
            ...
            $insert_data_man_mt1 = $DBKPI->get_sql("INSERT INTO sa_data_so (project_code_kpi,so_number_kpi,SB_existing_bu_price) VALUES ('$project_code_sb_mandays_mt1','$so_number','$mandays_mt1') ON DUPLICATE KEY UPDATE SB_existing_bu_price='$mandays_mt1'");
        ...
        do {
            ...
            // echo "++" . $service_type_sb2 . "-" . $DAY2 . "-" . $total_implementation_price_sb_mt . "-" . $bpd_price_sb_mt . "-" . $maintenance_package . "-" . $maintenance_addon_description;
            $insert_data_man_mt2 = $DBKPI->get_sql("INSERT INTO sa_data_so (project_code_kpi,so_number_kpi,SB_investment_bu_price) VALUES ('$project_code_sb_mandays_mt2','$so_number','$mandays_mt2') ON DUPLICATE KEY UPDATE SB_investment_bu_price='$mandays_mt2'");
        ...
        do {
            ...
            // echo "++" . $service_type_sb2 . "-" . $DAY2 . "-" . $total_implementation_price_sb_mt . "-" . $bpd_price_sb_mt . "-" . $maintenance_package . "-" . $maintenance_addon_description;
            $insert_data_man_wr1 = $DBKPI->get_sql("INSERT INTO sa_data_so (project_code_kpi,so_number_kpi,SB_warranty_cisco_price) VALUES ('$project_code_sb_mandays_wr1','$so_number','$mandays_wr1') ON DUPLICATE KEY UPDATE SB_warranty_cisco_price='$mandays_wr1'");
        ...
        do {
            ...
            // echo "++" . $service_type_sb2 . "-" . $DAY2 . "-" . $total_implementation_price_sb_mt . "-" . $bpd_price_sb_mt . "-" . $maintenance_package . "-" . $maintenance_addon_description;
            $insert_data_man_wr2 = $DBKPI->get_sql("INSERT INTO sa_data_so (project_code_kpi,so_number_kpi,SB_warranty_noncisco_price) VALUES ('$project_code_sb_mandays_wr2','$so_number','$mandays_wr2') ON DUPLICATE KEY UPDATE SB_warranty_noncisco_price='$mandays_wr2'");
        ...
        do {
            ...
            // echo "++" . $service_type_sb2 . "-" . $DAY2 . "-" . $total_implementation_price_sb_mt . "-" . $bpd_price_sb_mt . "-" . $maintenance_package . "-" . $maintenance_addon_description;
            $insert_data_man_wr2 = $DBKPI->get_sql("INSERT INTO sa_data_so (project_code_kpi,so_number_kpi,SB_warranty_price) VALUES ('$project_code_sb_mandays_wr2','$so_number','$mandays_wr2') ON DUPLICATE KEY UPDATE SB_warranty_price='$mandays_wr2'");
        ...
        do {
            ...
            // echo "++" . $service_type_sb2 . "-" . $DAY2 . "-" . $total_implementation_price_sb_mt . "-" . $bpd_price_sb_mt . "-" . $maintenance_package . "-" . $maintenance_addon_description;
            $insert_data_man_wr2 = $DBKPI->get_sql("INSERT INTO sa_data_so (project_code_kpi,so_number_kpi,SB_outsourcing_implementation_price) VALUES ('$project_code_sb_addon_imp','$so_number','$addon_imp') ON DUPLICATE KEY UPDATE SB_outsourcing_implementation_price='$addon_imp'");
        ...
        do {
            ...
            // echo "++" . $service_type_sb2 . "-" . $DAY2 . "-" . $total_implementation_price_sb_mt . "-" . $bpd_price_sb_mt . "-" . $maintenance_package . "-" . $maintenance_addon_description;
            $insert_data_man_wr2 = $DBKPI->get_sql("INSERT INTO sa_data_so (project_code_kpi,so_number_kpi,SB_outsourcing_maintenance_price) VALUES ('$project_code_sb_addon_mt','$so_number','$addon_mt') ON DUPLICATE KEY UPDATE SB_outsourcing_maintenance_price='$addon_mt'");
        ...
        do {
            ...
            if (empty($check[0]['so_number'])) {
                $insert_data_est_wr = $DBKPI->get_sql("UPDATE sa_data_so SET WR_status_project='$status_project' WHERE so_number_kpi='$so_number'");
            } else {
                $insert_data_est_wr = $DBKPI->get_sql("UPDATE sa_data_so SET WR_status_project='$status_project' WHERE so_number_kpi='$so_number'");
            }
        ...
        do {
            ...
            if (empty($check[0]['so_number_kpi'])) {
                $insert_data_est_wr = $DBKPI->get_sql("UPDATE sa_data_so SET WR_status_project='$status_project' WHERE so_number_kpi='$order_number'");
            } else {
                $insert_data_est_wr = $DBKPI->get_sql("UPDATE sa_data_so SET WR_status_project='$status_project' WHERE so_number_kpi='$order_number'");
            }
        ...
        do {
            ...
            if (empty($check[0]['so_number_kpi'])) {
                ...
                $insert_data_est_wr = $DBKPI->get_sql("UPDATE sa_data_so SET WR_status_project = '$status_project' WHERE so_number_kpi='$project_code'");
            } else {
                $insert_data_est_wr = $DBKPI->get_sql("UPDATE sa_data_so SET WR_status_project = '$status_project' WHERE so_number_kpi='$project_code'");
            }
        ...
        do {
            ...
            if (empty($check[0]['so_number_kpi'])) {
                ...
                $insert_data_est_wr = $DBKPI->get_sql("UPDATE sa_data_so SET kpi_status='$kpi_status', periode='$periode[0]', WR_start_assignment_implementation='$start_date_project', WR_bast_date_project_implementation='$bast_date_project', WR_bast_date_actual_project_implementation='$bast_date_actual', WR_project_estimation_implementation='$project_duration_wrike', WR_duration_actual_implementation='$project_duration_actual_wrike', status='$status' WHERE so_number_kpi='$so_number'");
            } else if ($so_number != '' || $so_number != null) {
                $so_number = $check[0]['so_number_kpi'];
                if (empty($bast_date_actual)) {
                    $insert_data_est_wr = $DBKPI->get_sql("UPDATE sa_data_so SET kpi_status='$kpi_status', periode='$periode', WR_start_assignment_implementation='$start_date_project', WR_bast_date_project_implementation='$bast_date_project', WR_bast_date_actual_project_implementation='$bast_date_actual', WR_project_estimation_implementation='$project_duration_wrike', WR_duration_actual_implementation='$project_duration_actual_wrike', status='$status' WHERE so_number_kpi='$so_number'");
                } else {
                    $insert_data_est_wr = $DBKPI->get_sql("UPDATE sa_data_so SET kpi_status='$kpi_status', periode='$periode[0]', WR_start_assignment_implementation='$start_date_project', WR_bast_date_project_implementation='$bast_date_project', WR_bast_date_actual_project_implementation='$bast_date_actual', WR_project_estimation_implementation='$project_duration_wrike', WR_duration_actual_implementation='$project_duration_actual_wrike', status='$status' WHERE so_number_kpi='$so_number'");
                }
            }
        ...
        // $query_get_wrike_mandays = $DBWR->get_sql("SELECT so_number,SUM((jumlah_actual/60/8)*value) AS mandays_actual_wrike,SUM((duration_planning/60/8)*value) AS mandays_plan_wrike FROM `sa_view_dashboard_wrike_resource` GROUP BY project_code");
        ...
        //     // echo "++" . $service_type_sb2 . "-" . $DAY2 . "-" . $total_implementation_price_sb_mt . "-" . $bpd_price_sb_mt . "-" . $maintenance_package . "-" . $maintenance_addon_description;
        //     $insert_data_man_wr = $DBKPI->get_sql("INSERT INTO sa_data_kp (so_number_kpi,WR_mandays_actual_implementation,WR_mandays_plan_implementation) VALUES ('$project_code_wr','$mandays','$mandays_plan') ON DUPLICATE KEY UPDATE WR_mandays_actual_implementation='$mandays', WR_mandays_plan_implementation='$mandays_plan'");
        // } while ($row13 = $res13->fetch_assoc());
        // $sqll15 = $DBWR->get_sql("SELECT project_code,SUM((jumlah_actual/60/8)*value) AS mandays_actual_wrike,SUM((duration_planning/60/8)*value) AS mandays_plan_wrike FROM `sa_view_dashboard_wrike_resource` GROUP BY project_code");
        // $no = 1;
        // do {
        //     $project_code_wr = $sqll15[0]['project_code'];
        //     $mandays = number_format($sqll15[0]['mandays_actual_wrike'], 5, ".", "");
        //     $mandays_plan = number_format($sqll15[0]['mandays_plan_wrike'], 5, ".", "");
        //     // echo $mandays . "<br>";
        //     // echo $mandays_plan . "test <br>";
        //     $insert_data_man_wr = $DBKPI->get_sql("INSERT INTO sa_data_kp (project_code_kpi,WR_mandays_actual_implementation,WR_mandays_plan_implementation) VALUES ('$project_code_wr','$mandays','$mandays_plan') ON DUPLICATE KEY UPDATE WR_mandays_actual_implementation='$mandays', WR_mandays_plan_implementation='$mandays_plan'");
        // } while ($sqll15[0] = $sqll15[1]->fetch_assoc());
        ...
        $query_get_cr_data = $DBCR->get_sql("SELECT a.project_code,a.so_number,SUM(b.mandays_value) AS mandays_cr from sa_general_informations a left join sa_mandays b on a.cr_no=b.cr_no WHERE a.change_request_approval_type='submission_approved' OR a.change_request_approval_type2='submission_approved' GROUP BY a.cr_no");
        ...
        do {
            ...
            if ($so_cr == null) {
                $insert_data_man_wr = $DBKPI->get_sql("UPDATE sa_data_so SET CR_mandays_implementation='$mandays_cr' WHERE project_code_kpi='$project_code_cr'");
            } else {
                $so_number = $row14['so_number'];
                $insert_data_man_wr = $DBKPI->get_sql("INSERT INTO sa_data_so (project_code_kpi,so_number_kpi,CR_mandays_implementation) VALUES ('$project_code_cr','$so_number','$mandays_cr') ON DUPLICATE KEY UPDATE CR_mandays_implementation='$mandays_cr'");
            }
        ...
        do {
            ...
            $bast = $ambil_status_so[0]['WR_duration_actual_implementation'];
            ...
            $insert = $DBKPI->get_sql("UPDATE sa_data_so SET kpi_status='$kpi_status', status='$stat' WHERE so_number_kpi='$so_number'");
        } while ($ambil_status_so[0] = $ambil_status_so[1]->fetch_assoc());
    } else if ($_GET['act'] == "test") {
        //  $test = 27/03/2023 - 13/02/2023;
        //  echo $test;
...
echo "Time : " .$timeline . "</br>";
            } else {
echo "Type : " .$timeline . "</br>";
            }
