...
?>
        ...
        <div class="col-lg-12">
            ...
            $query = "SELECT sur.survey_id, so_number, pic_name, pic_email, pic_phone, project_type, customer_company_name, type_of_service, trx.project_code, trx.project_title, sur.survey_link, sur.status, main_rating, main_essay, main_engineer, sur.created_datetime, sur.reply_datetime, rating_average
                ...
                where YEAR(sur.created_datetime) = $year $surveycust $surveytype LIMIT 0,100";
            ...
            $header = [
                'Project Tittle', 'Project Code (SO Number)', 'Project Type', 'Project Category', 'Customer Name', 'PIC', 'PIC Phone', 'Status', 'Created Date', 'Reply Date', 'Nilai', 'Resource Terbaik Menurut Customer', 'Project Memiliki Perencanaan Kerja yang Baik', 'Project Team Terhadap Permintaan Customer', 'Penanganan insiden', 'Permintaan Perubahan (Change Request): Atas permintaan yang disetujui', 'Dokumentasi project disampaikan kepada customer dengan lengkap', 'Customer dengan umum puas dengan Hasil Kerja', 'Pekerjaan team project sudah sesuai persyaratan dalam dokumen RFP/RKS', 'Solusi yang diberikan PT. Mastersystem infotama memenuhi kebutuhan bisnis perusahaan', 'Silahkan masukan saran perubahan anda untuk meningkatkan kinerja team project', 'Action'
            ];
            $header1 = '';
...
<script>
    $(document).ready(function() {
        ...
        $('#filterbtn').click(function() {
            ...
            window.location.href = 'index.php?mod=dashboard&sub=dashboard_survey' + years + custs + tipes + temps;
        });
    ...
    // pengisian survey
    <?php
    $query2 = "select count(if(status='Active' and project_type is not null, 1, null)) as active, count(if(status='default' AND project_type is not null, 1, null)) as inactive, count(if(status='submitted' AND project_type is not null, 1, null)) as submitted from sa_survey";
