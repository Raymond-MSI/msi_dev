...
<!-- Sidebar - Brand -->
    ...
    <?php if($_SESSION['Microservices_UserLevel'] == 'Administrator' || $_SESSION['Microservices_UserLevel'] == 'Super Admin') { ?>
        ...
        <!-- <li>
            <?php
                // $condition = 'parent=17 AND published=1';
                // $tblname = 'cfg_menus';
                // $menusidebar = $DB->get_data($tblname, $condition, $order);
                // $dmenusidebar = $menusidebar[0];
                // $qmenusidebar = $menusidebar[1];
            ?>
            <div id="collapseDesign" class="collapse" aria-labelledby="headingTwo" data-parent="#accordionSidebar">
                <div class="bg-white py-2 collapse-inner rounded">
                    <h6 class="collapse-header">Custom Components:</h6>
                    <?php //do { ?>
                        <a class="collapse-item" href="<?php //echo $dmenusidebar['link']; ?>"><?php //echo $dmenusidebar['title']; ?></a>
                    <?php //} while ($dmenusidebar = $qmenusidebar->fetch_assoc()); ?>
                </div>
            </div>
        </li>  -->
