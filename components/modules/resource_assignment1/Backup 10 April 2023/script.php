...
// echo "$projectCode - $noSO - $email - $roles - $status - $startProgress - $endProgress - $description - $emailSession - $customer - $projectName <br/>";
...
if($roles != '' && $roles != ' - '){
    ...
    if($rowCountRes > 0){
        ...
        if($rowcount > 0) {
            ...
            $sqlInsert = "INSERT INTO sa_temporary_resource (`project_code`,`project_id`,`no_so`, `project_type`, `customer_name`,`project_name`,`resource_email`,`roles`,`project_roles`,`start_progress`,`end_progress`,`status`, `description`, `created_by`)
            VALUES ('$projectCode', '$projectId', '$noSO', '$projectType', '$customer', '$projectName', '$email', '$roles', '$projectRoles', '$startProgress', '$endProgress', '$status', '$description', '$emailSession')";
