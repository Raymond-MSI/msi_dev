...
if ((isset($property)) && ($property == 1)) {
    ...
    if (USERPERMISSION == "7b7bc2512ee1fedcd76bdc68926d4f7b" || USERPERMISSION == "dbf36ff3e3827639223983ee8ac47b42" || USERPERMISSION == "5898299487c5b9cdbe7d61809fd20213") {
        $mdlname = "KPI_BOARD";
        $DBKPILog = get_conn($mdlname);
        $datakpi = $DBKPILog->get_sqlV2("SELECT * FROM sa_kpi_board WHERE status_pic LIKE '%pending%'");
        $project_code = $datakpi[0]['so_number'];

        // var_dump($project_code);
        // die;
?>
        <input type="hidden" id="datakpi" value="<?php echo $datakpi[0]['so_number']; ?>">
        <script>
                        ...
                        },
                        {
                            ...
                            action: function() {
                                ...
                                if (complain_id == null) {
                                    ...
                                    window.location.href = "index.php?mod=log_complain&act=edit&complain_id=" + complain_id + "&ProjectCode=" + project_code + "&So_Number=" + so_number + "&submit=Submit";
                                }
                            }
                        },
                        {
                            ...
                            action: function() {
                                ...
                                var project_code = table.cell(rownumber, 1).data();
                                var so_number = table.cell(rownumber, 2).data();
                                var datakpi = document.getElementById('datakpi').value;
                                if (complain_id == null) {
                                    alert("Select Data");
                                } else {
                                    window.location.href = "index.php?mod=log_complain&act=delete&complain_id=" + complain_id + "&ProjectCode=" + project_code + "&submit=Submit";
                                }
                            }
                        },
                        {
