...
if (isset($_POST['add'])) {
    ...
    if ($checkup == FALSE) {
        $insert = sprintf(
            "(`employee_name`,`jabatan`,`division`,`start_date`,`end_date`,`duration`,`manager`,`leader`,`reason`,`status`,`entry_by`,`entry_date`,`performed_by`,`category`,`project_code`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
    ...
    $update = sprintf(
        "`status`='request approved', `flag_approval`=%s, `overtime_approval_by`=%s, `performed_by`=%s",
...
if (isset($_POST['add_leave']) || isset($_POST['edit_leave'])) {
    if (isset($_POST['add_leave'])) {
        ...
        $mysql = sprintf(
            "INSERT INTO `sa_trx_edo_leave`
                ...
                (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            GetSQLValueString(addslashes($MyName), "text"),
        ...
        // if ($rs === false) {
        //     echo "Error executing query: " . $DBHCM->errorInfo(); // Ganti dengan metode yang sesuai untuk menangkap kesalahan
        //     die;
        // }


        // if ($rs) {
        //     // SQL was successful, prepare the email
        //     $subject = "[EDO] Apply of EDO";
        //     $msgx = sprintf(
        //         "<p>To Leaders, </p>
        // <p>I hereby submit my EDO leave application for your approval as follows: </p>
        // <p>Name : %s <br/>
        // Email : %s <br/>
        // Leader 1 Email : %s <br/>
        // Leader 2 Email : %s <br/>
        // Leave Period: %s to %s <br/>
        // Leave Status: %s <br/>
        // Entry By: %s <br/>
        // Entry Date: %s <br/>
        // Reason: %s </p>
        ...
        //         GetSQLValueString($MyEmail, 'defined', $MyEmail, $MyEmail),
        //         GetSQLValueString($_POST['leader1_email'], 'defined', $_POST['leader1_email'], $_POST['leader1_email']),
        //         GetSQLValueString($_POST['leader2_email'], 'defined', $_POST['leader2_email'], $_POST['leader2_email']),
        //         GetSQLValueString(date('d-M-Y', strtotime($_POST['leave_start'])), 'date'),
        //         GetSQLValueString(date('d-M-Y', strtotime($_POST['leave_end'])), 'date'),
        //         GetSQLValueString("leave submitted", 'text'),
        //         GetSQLValueString($MyName, 'defined', $MyName, $MyName),
        //         GetSQLValueString(date("d-M-Y G:i:s"), 'date'),
        //         GetSQLValueString($_POST['reason'], 'text'),
        //         GetSQLValueString(date('d-M-Y', strtotime('+7 day')), 'text'),
        //         GetSQLValueString($MyName, 'defined', $MyName, $MyName)
        //     );

        //     // Specify headers for the email
        //     $headers = "MIME-Version: 1.0" . "\r\n";
        //     $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
        //     $headers .= "From: <$MyEmail>" . "\r\n";

        //     // Combine Leader emails into the To header
        //     $to = $_POST['leader1_email'] . ", " . $_POST['leader2_email'];

        //     // Send the email
        //     if (mail($to, $subject, $msgx, $headers)) {
        //         echo "Email sent successfully.";
        //     } else {
        //         echo "Email sending failed.";
        //     }
        // } else {
        //     echo "Error saving leave request.";
        // }
    ...
    foreach ($_POST['edo'] as $edo) {
        ...
        // {
        $mysql = sprintf(
            "INSERT INTO `sa_trx_edo_leave_detail`(`leave_id`, `edo_id`) VALUES (%s,%s)",
