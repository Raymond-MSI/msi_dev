    ...
    <tbody>
        ...
        ?>
                <tr>
                    ...
                    <?php
                    $get_resource_id = $DBWRIKE->get_sqlV2("SELECT * FROM sa_contact_user WHERE email LIKE '%" . $dtKPI[0]['resource_email'] . "%'");
                    $resource_id = $get_resource_id[0]['id'];
                    $get_project_id = $DBWRIKE->get_sqlV2("SELECT * FROM sa_wrike_project_list WHERE no_so='$so_number' AND project_type='MSI Project Implementation'");
                    $prodPlan = $DBGC->get_sqlV2("SELECT COUNT(a.task_name) AS total_task FROM sa_preschedule a left join sa_schedule b ON a.google_event_id=b.event_id WHERE a.project_id='" . $get_project_id[0]['id'] . "' AND a.resource_email='" . $dtKPI[0]['resource_email'] . "' AND b.response_status <>'declined'");
                    $prodActual = $DBGC->get_sqlV2("SELECT COUNT(a.project_id) as total_update FROM sa_preschedule a left join sa_schedule b ON a.google_event_id=b.event_id WHERE a.project_id = '" . $get_project_id[0]['id'] . "' AND b.attendees_email='" . $dtKPI[0]['resource_email'] . "' AND b.response_status='accepted' AND b.diff_time<>0");
                    ...
                    <!-- <td><?php //echo $productivity[0]['total_task'] . '&nbsp;&nbsp;|&nbsp;&nbsp;' . $productivity[0]['update_task']; 
                                ?></td> -->
                    <td><?php echo $prodPlan[0]['total_task'] . '&nbsp;&nbsp;|&nbsp;&nbsp;' . $prodActual[0]['total_update']; ?></td>
                    <td><?php echo $KPIUser[0]['role']; ?></td>
                    <td><?php echo number_format($KPIUser[0]['nilai_ideal'], 2); ?></td>
                    <td><?php echo number_format($KPIUser[0]['nilai_aktual'], 2); ?></td>
                    <td><?php echo number_format(str_replace(",", ".", $KPIUser[0]['nilai_akhir_ideal']), 2); ?></td>
                    <td><?php echo number_format(str_replace(",", ".", $KPIUser[0]['nilai_akhir_aktual']), 2); ?></td>
                </tr>
        <?php
