...
$DBPTIVITY = get_conn("PRODUCTIVITY");
?>
<script>
...
<div class="alert alert-light fs-4" role="alert">
    <?php echo "Status Review : " . $_GET['status']; ?>
</div>
        ...
        <table class="table table-striped table-bordered" id="tblKPIProject">
            ...
            <tbody>
                ...
                do {
                    ...
                    // SUM(
                    //     IF(
                    //         DAYOFWEEK(`task_date`) BETWEEN 2 AND 6,
                    //         1,
                    //         IF(
                    //             work_on_weekends = 1,
                    //             1,
                    ...
                    // SUM(
                    //     IF(
                    //         status_calender = '' OR status_calender IS NULL,
                    //         IF(
                    //             permalink_gcalender = '' OR permalink_gcalender IS NULL,
                    //             IF(
                    //                 DAYOFWEEK(`task_date`) BETWEEN 2 AND 6,
                    //                 1,
                    //                 IF(
                    ...
                    //     )
                    // ) as `none` 
                    // FROM sa_trx_productivity 
                    // WHERE 
                    //     project_type='MSI Project Implementation'
                    //     AND resource_email LIKE %s
                    //     AND (
                    //         DAYOFWEEK(`task_date`) BETWEEN 2 AND 6
                    //         OR `status_calender`=1
                    //     )
                    // GROUP BY resource_email
                    // ORDER BY resource_email
                    // ",
                    //                     // GetSQLValueString($start_date, "date"),
                    //                     // GetSQLValueString($end_date, "date"),
                    //                     GetSQLValueString($resource_list[0]['Nama'], "text")
                    //                 );
                    //                 $rsProductivity = $DBPTIVITY->get_sql($mysql);
                    $get_resource_id = $DBWRIKE->get_sqlV2("SELECT * FROM sa_contact_user WHERE email LIKE '%$resource%'");
                    $resource_id = $get_resource_id[0]['id'];
                    $get_project_id = $DBWRIKE->get_sqlV2("SELECT * FROM sa_wrike_project_list WHERE order_number='" . $resource_list[0]['order_number'] . "' AND project_type='" . $resource_list[0]['project_type'] . "'");
                    $get_jumlah = $DBGC->get_sqlV2("SELECT COUNT(a.task_name) AS jumlah_task FROM sa_preschedule a left join sa_schedule b ON a.google_event_id=b.event_id WHERE a.project_id='" . $get_project_id[0]['id'] . "' AND a.resource_email='$resource' AND b.response_status <>'declined'");
                    $get_updated = $DBGC->get_sqlV2("SELECT COUNT(a.project_id) as updated_task FROM sa_preschedule a left join sa_schedule b ON a.google_event_id=b.event_id WHERE a.project_id = '" . $get_project_id[0]['id'] . "' AND b.attendees_email='$resource' AND b.response_status='accepted' AND b.diff_time<>0");
                    $get_progress = $DBWRIKE->get_sqlV2("SELECT * FROM sa_resource_assignment WHERE order_number='" . $resource_list[0]['order_number'] . "' AND resource_email LIKE '%$resource%'");
                ...
                ?>
                    <tr>
                        <td><span class="text-nowrap"><?php echo $resource_list[0]['project_code']; ?>&nbsp;|&nbsp;<?php echo $resource_list[0]['so_number']; ?><br /><span style="font-size:12px"><?php echo $resource_list[0]['project_name']; ?><br /><?php echo $resource_list[0]['role'] . " ($start_progress% - $end_progress%)"; ?></span></td>
                        <td class="text-center text-nowrap"><?php echo $get_jumlah[0]['jumlah_task'] . '&nbsp;&nbsp;|&nbsp;&nbsp;' . $get_updated[0]['updated_task']; ?></td>
                        <!-- <td class="text-center text-nowrap"><?php echo $rsProductivity[0]['plan'] . '&nbsp;&nbsp;|&nbsp;&nbsp;' . $rsProductivity[0]['accepted1']; ?></td> -->
