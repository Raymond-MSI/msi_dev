                ...
                <div class="dropdown no-arrow" id="<?php echo htmlspecialchars($tiptool); ?>">
                    ...
                    <div class="dropdown-menu dropdown-menu-left shadow animated--fade-in" aria-labelledby="dropdownMenuLink">
                        ...
                        foreach ($listData as $item) {
                            $resource = isset($_GET['project_manager']) ? "&resource=" . urlencode($_GET['project_manager']) : '';
                        ?>
...
if ($username == 'hendri@mastersystem.co.id' || $username == 'chrisheryanda@mastersystem.co.id' || $username == 'syamsul@mastersystem.co.id' || $username == 'lucky.andiani@mastersystem.co.id' || $username == 'aceng.zakariya@mastersystem.co.id' || $username == 'anggi.fachrizal@mastersystem.co.id' || $username == 'Raymond.Raphael@mastersystem.co.id' || $username == 'anita.angelina@mastersystem.co.id' || $username == 'fortuna@mastersystem.co.id' || $username == 'iwan@mastersystem.co.id' || $username == 'cassendra@mastersystem.co.id') {
    ...
    $get_project = $DBKPI->get_sqlV2("SELECT a.*,c.owner_email FROM sa_kpi_so_wr a left join sa_master_maintenance_date b ON a.order_number=b.order_number left join sa_wrike_integrate.sa_wrike_project_list c ON a.order_number=c.order_number WHERE a.project_type LIKE '%Maintenance%' AND c.project_type LIKE '%Maintenance%' AND ($resourceEmail) $kpiStatus");
    $get_project2 = $DBKPI->get_sqlV2("SELECT a.*,c.owner_email FROM sa_kpi_so_wr a left join sa_master_maintenance_date b ON a.order_number=b.order_number left join sa_wrike_integrate.sa_wrike_project_list c ON a.order_number=c.order_number WHERE a.project_type LIKE '%Maintenance%' AND c.project_type LIKE '%Maintenance%' AND ($resourceEmail) $kpiStatus");
    $get_project3 = $DBKPI->get_sqlV2("SELECT a.*,c.owner_email FROM sa_kpi_so_wr a left join sa_master_maintenance_date b ON a.order_number=b.order_number left join sa_wrike_integrate.sa_wrike_project_list c ON a.order_number=c.order_number WHERE a.project_type LIKE '%Maintenance%' AND c.project_type LIKE '%Maintenance%' AND ($resourceEmail) $kpiStatus");
}
    ...
    <div class="row mb-3">
        ...
        <div class="col-lg-9">
            <div class="d-flex flex-row">
                <?php if ($username == 'chrisheryanda@mastersystem.co.id' || $username == 'syamsul@mastersystem.co.id' || $username == 'lucky.andiani@mastersystem.co.id' || $username == 'aceng.zakariya@mastersystem.co.id' || $username == 'anggi.fachrizal@mastersystem.co.id' || $username == 'Raymond.Raphael@mastersystem.co.id' || $username == 'anita.angelina@mastersystem.co.id' || $username == 'fortuna@mastersystem.co.id' || $username == 'iwan@mastersystem.co.id' || $username == 'cassendra@mastersystem.co.id') { ?>
                    <div class="">
                        <div
                            ...
                            <span
                                ...
                                <?php
                                    ...
                                    <select id="projectCodee" class="form-control form-control-sm" name="projectCodee" onchange="onChangeFunction()">
                                        <?php $email = $_GET['project_manager'];
                                        $get_data = $DBKPI->get_sqlV2("SELECT * FROM sa_user WHERE resource_email='$email'"); ?>
                                        <option value="<?php echo $_GET['project_manager']; ?>"><?php echo $get_data[0]['Nama']; ?></option>
                                        <?php
                                        $select_all = $DBKPI->get_sqlV2("SELECT * FROM sa_user WHERE project_type LIKE '%Maintenance%' AND role='PIC Maintenance' GROUP BY Nama");
                                        while ($row = $select_all[1]->fetch_assoc()) {
                                            $nama = $row['Nama'];
...
<body>
    <div class="row">
                ...
                <tbody>
                    ...
                    if ($get_project[2] > 0) {
                        ...
                        foreach ($projects as $project) {
                            echo "<tr>";
                            echo "<td>{$project['name']} <br><span class='text-nowrap' style='font-size:12px'>{$project['customer_name']} |</span> <span class='text-nowrap' style='font-size:12px'>{$project['code']} |</span> <span class='text-nowrap' style='font-size:12px'>{$project['so_number']} |</span> <span class='text-nowrap' style='font-size:12px'>{$project['order_number']} |</span> <span class='text-nowrap' style='font-size:12px'>{$project['manager']} |</span> <span class='text-nowrap' style='font-size:12px'>{$project['maintenance']} |</span> <span style='font-size:12px'>{$project['backdate']}</span></td>";
                            ...
                            for ($year = 2019; $year <= 2028; $year++) {
                                ...
                                if (empty($check[0]['tahun_review'])) {
                                    ...
                                    $nilai = '';
                                    if ($year > $start && $year <= $end && $year == $tahun_review) {
                                        $class = 'highlight2';
                                        $nilai = $check[0]['total_cte'] . "%";
                                    } else if ($year == $start && $year == $end && $year == $tahun_review) {
                                        $class = 'highlight2';
                                        $nilai = $check[0]['total_cte'] . "%";
                                    }
                                    echo "<td class='$class' style='text-align:center; color:white;'>$nilai</td>";
                                }
                            }
                            echo "</tr>";
                        }
        ...
        <div class="row mb-3" style="display: flex; justify-content: center; align-items: center;">
            <table id="legendTable" style="border-spacing: 10px; width: 50%;">
                <tr>
                    <td style="background-color: lightblue; width: 40px; height: 40px;"></td>
                    <td style="padding-left: 10px;">Not yet reviewed</td>
                    <td style="background-color: green; width: 40px; height: 40px; margin-left: 20px;"></td>
                    <td style="padding-left: 10px;">Reviewed</td>
                </tr>
            </table>
        </div>
    </div>
                                ...
                                <table class="display" id="ProjectDetail" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        while ($row = $get_project3[1]->fetch_assoc()) {
                                            ...
                                            if (empty($get_data_so[0]['SB_maintenance_price'])) {
                                                $value = "Rp. 0.";
                                            } else {
                                            ...
                                            if (empty($get_data_ticket[0]['order_number'])) {
                                                ...
                                                $get_jumlah_gabungan = $DBKPI->get_sql("SELECT order_number,COUNT(project_code) as jumlah_report FROM sa_maintenance_date_kpi WHERE order_number='$order_number' AND maintenance_name_file LIKE '%pmr%'");
                                                ...
                                                if (empty($get_jumlah_gabungan[0]['jumlah_report'])) {
                                                    $aktual_gabungan = 0;
                                                } else {
                                                    $aktual_gabungan = $get_jumlah_gabungan[0]['jumlah_report'];
                                                }
                                                ...
                                                $jumlah_gabungan = $get_data_ticket[0]['total_pmr_date'] . " - " . $aktual_gabungan;
                                            }
                                            ...
                                            $baseline_gabungan = $DBKPI->get_sqlV2("SELECT * FROM sa_maintenance_date WHERE order_number='$order_number' AND id_date LIKE '%pmr%'");
                                            $gabungan_maintenance = $DBKPI->get_sqlV2("SELECT a.*,b.maintenance_aktual_date,b.maintenance_category from sa_maintenance_date a left join sa_maintenance_date_kpi b ON a.order_number=b.order_number AND a.id_date=b.maintenance_name_file WHERE a.id_date LIKE '%pmr%' AND a.order_number='$order_number'");
                                            if (isset($baseline_gabungan[0]['order_number'])) {
                                                $a = 0;
                                                $table_gabungan = '';
                                                while ($row = $gabungan_maintenance[1]->fetch_assoc()) {
                                                    $a++;
                                                    $gabungan_plan_date = date("d-M-Y", strtotime($row['date']));
                                                    if (empty($row['maintenance_aktual_date'])) {
                                                        $gabungan_aktual_date = 'File belum diupload';
                                                        $gabungan_category = '';
                                                    } else {
                                                        $gabungan_aktual_date = date("d-M-Y", strtotime($row['maintenance_aktual_date']));
                                                        $gabungan_category = $row['maintenance_category'];
                                                    }
                                                    $table_gabungan .= "<tr><td>$a</td><td>$gabungan_plan_date</td><td>$gabungan_aktual_date</td><td>$gabungan_category</td></tr>";
                                                }
                                            } else {
                                                $table_gabungan = "<tr><td colspan='4'>No data available</td></tr>";
                                            }
                                        ...
                                        ?>
                                            <tr data-toggle="modal" data-target="#myModal"
                                                data-reportgabung="<?php echo $jumlah_gabungan; ?>"
                                                data-tablegabungan="<?php echo $table_gabungan; ?>"
                                                ...
                                                <td><?php echo $start . " ~ <span class='text-nowrap'>" . $end . "</span>"; ?></td>
                                                <td><?php echo $total_cte; ?></td>
                                                <td><?php echo $weighted_value; ?></td>
                                                <td><?php echo $kpi_status; ?></td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
                                </table>
                        ...
                        <div class="card-body">
                            <table class="display compact hover" id="ProjectDetail3" style="width:100%">
                                ...
                                <tbody>
                                    <?php
                                    if (empty($kpi_status)) {
                                        $kpiStatus = "AND kpi_status='Not yet reviewed'";
                                    } else {
                                        $kpiStatus = "AND kpi_status='$kpi_status'";
                                    }
                                    ...
                                    WHERE project_type LIKE '%Maintenance%' $kpiStatus
                                    GROUP BY
                                        `resource_name`,`periode_kpi`,`kpi_status`");
                                    if ($rsResources[2] > 0) {
                                        $tableResources = '';
                                        do {
                                    ?>
                        ...
                        <div class="row mb-10">
                            <div class="table-responsive">
                                <table class="display table table-bordered" id="ProjectDetail" style="width:100%">
                                    <thead>
                                        <tr class="text-center">
                                            <th>Jumlah Preventive & Maintenance Report</th>
                                            <th>Jumlah Report</th>
                                    ...
                                    <tbody>
                                        <tr>
                                            <td style="text-align: center;"><span id="modalReportGabungan"></span></td>
                            ...
                            <div class="row mb-3">
                                <div class="table-responsive">
                                    <table class="display table table-bordered" id="ProjectDetail" style="width:100%">
                                        <thead>
                                            <tr>
                                                <th colspan="4"><b>Preventive & Maintenance Report</b></th>
                                            </tr>
                                            <tr>
                                                <th>Preventive & Maintenance Report ke-</th>
                                                <th>Tanggal Plan</th>
                                                <th>Tanggal Aktual</th>
                                                <th>Status</th>
                                            </tr>
                                        </thead>
                                        <tbody id="modalTableGabungan">
                                        </tbody>
                                    </table>
                                </div>
                            </div>
    ...
    <script>
        ...
        $("#projectManager").on('change', function() {
            ...
            var customer_name = $('#customer').val();
            window.location = window.location.pathname + "?mod=kpi_project&sub=<?php echo $_GET['sub']; ?>&project_manager=" + resource;
        })
    </script>
    <script>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('#ProjectDetail3 tbody').on('dblclick', 'tr', function() {
                ...
                $("#div2").load("components/modules/kpi_project/dashboard_kpi_resource_mt.php?resource=" + exp2[0] + "&status_review=" + review);
...
<script>
    $('#myModal').on('show.bs.modal', function(event) {
        ...
        var tablegabungan = button.data('tablegabungan');
        var reportgabung = button.data('reportgabung');
        ...
        modal.find('#modalTableGabungan').html(tablegabungan);
        modal.find('#modalReportGabungan').text(reportgabung)
        ...
        $('#openSecondModal').data('tablegabungan', tablegabungan);
    });
    ...
    $('#openSecondModal').on('click', function() {
        ...
        var tablegabungan = $(this).data('tablegabungan');
        ...
        setTimeout(function() { // Wait for the first modal to be completely hidden
            ...
            $('#secondModal').find('#modalTableGabungan').html(tablegabungan);
