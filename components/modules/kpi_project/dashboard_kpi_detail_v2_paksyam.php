            ...
            do { 
                ...
                <tr>
                    ...
                    <td class="text-nowrap">
                        ...
                        $tblname = 'user';
                        $condition = 'order_number = "' . $dtKPI[0]['order_number'] . '" AND Nama LIKE "%' . $dtKPI[0]['resource_email'] . '%"';
                    ...
                    <?php
                    $mysql = sprintf("SELECT COUNT(`sa_wrike_task`.`task_id`) AS `total_task` FROM `sa_wrike_project_list` LEFT JOIN `sa_wrike_assignment` ON `sa_wrike_project_list`.`id` = `sa_wrike_assignment`.`project_id` LEFT JOIN `sa_wrike_task` ON `sa_wrike_task`.`task_id` = `sa_wrike_assignment`.`task_id` AND `sa_wrike_task`.`project_id` = `sa_wrike_project_list`.`id` WHERE `sa_wrike_assignment`.`resource_email` LIKE %s AND `sa_wrike_integrate`.`sa_wrike_assignment`.`project_id` = %s; ",
                    ...
                    <td><?php echo $KPIUser[2]>0 ? $KPIUser[0]['role'] : ""; ?></td>
